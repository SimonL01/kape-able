...
for %%R in ("%JOBSDIR%\*.rc") do (
    ...
    if not defined _seen_%%~nR (
        set "_seen_%%~nR=1"
        set "RCC="
        for /f "usebackq delims=" %%v in ("%%~fR") do if not defined RCC set "RCC=%%v"
...
REM Clean up the per-run job directory
...
echo   %~nx0 NAME SRC DEST_ROOT ZIP_TAG            ^> %BYEL%Runs each CLI line, splits --target A,B,C%RST%
